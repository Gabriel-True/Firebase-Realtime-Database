...
No sub-nó `"animes"` definimos que o usuário logado poderá lê os dados, mas somente o usuário que estiver com a uid `"bZk15h2SU6T3u7FS3TSefJCs0mI3"` poderá gravar no database. Como um usuário admin, onde os usuários vão poder lê os dados, mas somente o usuário (admin) que estiver com essa uid poderá escrever/alterar no database.

No sub-nó `"posts"` definimos que qualquer usuário logado poderá lê e gravar no database.
...
Com certeza que você viu este símbolo no exemplo anterior. Tal como no PHP, ele serve para indicar que trata-se de uma variável. A ideia de declarar estas variáveis não é atribuir valores à elas, mas sim utilizar elas para comparar os seus valores com outros, tal como no exemplo anterior onde comparamos se o `uid` do usuário que está na base de dados (variável `$uid`) é igual ao uid do usuário que fez o login no firebase auth (variável `auth.uid`).
